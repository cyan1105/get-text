...
ap.add_argument("-i", "--image", type=str, default="images/2.jpg",
                help="path to input image")
ap.add_argument("-o", "--output", type=str, default="output/output2.jpg",
...
for ((startX, startY, endX, endY), text1) in results:
    ...
    text1 = "".join(["" if ord(c) < 128 else c for c in text1]).strip()
    print("OCR TEXT")
    print("========")
    print("{}\n".format(text1))
    draw.rectangle(xy=(startX, startY, endX, endY), fill=None, outline="red", width=2)
    draw.text((startX, startY - 20), text1, (0, 0, 255), font=fontStyle)

# 显示结果
output = cv2.cvtColor(np.asarray(output), cv2.COLOR_RGB2BGR)
